...
-   Specification sidebar - Here, readers can use sliders and drop-down menus to specify the following features of the plots:
    ...
    -   Number of facets and values of the second variable *Z* at which model predictions will be generated. By default, three facets are generated, for lower quartile, median and upper quartile values of *Z.*
...
We encourage users to accompany figures with text that guides users through the meaning of figures - specifically for alternative specifications of the figures.

[Generality]{.underline}

Here you can describe and justify the target context to which your inferences can be validly transferred (also known as the applicability domain). The aim of this tab is to encourage authors to draw conservative inferences, rather than make broad generalizations about undefined or ill-defined target contexts. We recommend writing a "Constraints on Generality" (CoG) statement, where you can clearly specify the assumptions and conditions that must hold for inferences to transfer.
